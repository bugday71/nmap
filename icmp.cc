...
int ICMP(int argc, char *argv[])
{
    ...
    if (argc < 4) 
    {
        printf("usage: %s <dst IP> [payload size] [count]\n", argv[1]);
        exit(0);
    }
    ...
    servaddr.sin_addr.s_addr = inet_addr ( daddr );;
    ...
    while (1)
    {
        ...
        icmp->checksum = 0;
        icmp->checksum = in_cksum2((unsigned short *)icmp, sizeof(struct icmphdr) + payload_size);
